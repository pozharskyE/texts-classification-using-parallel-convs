...
def sample_to_csv(path_to_json_gz_file_ds, size_by_group=5000, destination='./../../sampled_df.csv'):
    ...
    df = df.dropna()
    df = df[df['reviewText'].str.len() < 1500][df['reviewText'].str.len() > 20]

    group0 = df[df['overall'] == 1].sample(size_by_group)
    group0['overall'] = np.zeros_like(group0['overall'])

    group1 = df[df['overall'] == 5].sample(size_by_group)
...
if __name__ == '__main__':
    sample_to_csv('./data/Electronics_5.json.gz', size_by_group=5000, destination='./../../sampled_df.csv')
