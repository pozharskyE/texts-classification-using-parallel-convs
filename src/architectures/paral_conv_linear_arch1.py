...
class Parallel1DConvsLinearClass(nn.Module):
    def __init__(self, input_channels: int, output_size: int = 1, kernel_sizes: List[int] = [3, 4, 5], out_channels_per_kernel: int = 64, linear_hidden_sizes: List[int] = [256, 256, 1]):
        ...
        if linear_hidden_sizes[-1] == 1:
            ...
            raise ValueError(
                f'This error was raised because last hidden size is not >= 1. Please ensure, that last element in linear_hidden_sizes list, that you gave, is >= 1 (this is output size)')
